...
const ReaderProfile = () => {
  ...
  const fetchReaderData = async () => {
    try {
      ...
      // Fetch issued books
      const issuedResponse = await api.get(`/reader/issued-books/${user.email}`);
      setIssuedBooks(issuedResponse.data);

      // Fetch request history
      const historyResponse = await api.get(`/reader/request-history/${user.email}`);
          ...
          <div className="books-grid">
            {issuedBooks.map((book) => (
              <div key={book.IssueID} className="book-card">
                ...
                <p>Expected Return: {new Date(book.ExpectedReturnDate).toLocaleDateString()}</p>
